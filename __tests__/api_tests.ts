...
describe("AddressService", () => {
  it("Throw error if API key not provided", () => {
    ...
    expect(() => new AddressService(undefined as any)).toThrow(
...
describe("getNestedValue", () => {
  ...
  it("Get data", () => {
    ...
    expect(getNestedValue(testAddress, ["address", "streetNumber"])).toBe(
      "2/118"
    );
    expect(getNestedValue(testAddress, ["address", "streetName"])).toBe(
      "Haines Street"
    );
    expect(
      getNestedValue(testAddress, ["address", "municipalitySubdivision"])
    ).toBe("North Melbourne");
    expect(getNestedValue(testAddress, ["address", "municipality"])).toBe(
      "Melbourne"
    );
    expect(getNestedValue(testAddress, ["address", "countrySubdivision"])).toBe(
      "Victoria"
    );
    expect(getNestedValue(testAddress, ["address", "postalCode"])).toBe("3051");
    expect(getNestedValue(testAddress, ["address", "country"])).toBe(
      "Australia"
    );
  ...
  it("Get incomplete address", () => {
    ...
    expect(
      getNestedValue(incompleteAddress, ["address", "municipalitySubdivision"])
    ).toBe("");
  });
